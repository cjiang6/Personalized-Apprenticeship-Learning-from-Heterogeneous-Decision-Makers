"""
bnn copied over from bnn.py but translated for the toy environment
"""
...
class BNN_Gaussian(nn.Module):
    def __init__(self, hidden_size, stddev_prior, bias=True):
        ...
        self.stddev_prior = stddev_prior
        self.fc1 = GaussianLinear(2, hidden_size, stddev_prior, bias=bias)
        self.fc2 = GaussianLinear(hidden_size, hidden_size, stddev_prior, bias=bias)
        self.fc3 = GaussianLinear(hidden_size, 2, stddev_prior, bias=bias) # changed 10 to 2 because there are only 2 classes
    ...
    def forward_samples(self, x, y, nb_samples=3):
        ...
        for _ in range(nb_samples):
            ...
            y = y.view(len(y), -1)
            y_onehot = torch.LongTensor(len(y), 2).to(x.device) # again changed 10 to 2
            y_onehot.zero_()
