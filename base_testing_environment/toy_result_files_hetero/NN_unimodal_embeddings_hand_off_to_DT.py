...
for i in range(50):
    ...
    for each_t in range(chosen_schedule_start, chosen_schedule_start + 20):
        ...
        test_losses.append(loss.item())
        test_accs.append(acc.mean().item())
    print(MLP.get_bayesian_embedding().detach().numpy())
    per_schedule_test_accs.append(np.mean(test_accs))
    test_distributions[schedule_num] = list(MLP.get_bayesian_embedding().detach().clone().numpy())  # very ugly
...
counter = 0
new_tensor = []
for n,i in enumerate(x.clone()):
    if n % 20 == 0 and n != 0:
        counter += 1
    new_tensor.append(torch.cat([i,torch.Tensor(distributions[counter])]))
for j,i in enumerate(new_tensor.copy()):
    new_tensor[j] = np.array(i)
x = new_tensor
clf.fit(x, y)

new_tensor = []
counter = 0
for n,i in enumerate(x_test.clone()):
    if n % 20 == 0 and n != 0:
        counter += 1

    new_tensor.append(torch.cat([i,torch.Tensor(test_distributions[counter][0])]))
for j,i in enumerate(new_tensor.copy()):
    new_tensor[j] = np.array(i)
x_test = new_tensor
