...
def train(bnn, train_data, test_data, nb_samples,
    ...
    for epoch in range(nb_epochs):
        for phase, loader in loaders.items(): # basically loops once for train, once for test
            nb_correct = 0
            total_nll_loss = 0.
            total_kl_loss = 0.
            if phase == "train":
                nb_total = len(train_data) # total training images
            else:
                nb_total = len(test_data)
            for batch_idx, (x, y_true) in enumerate(loader): # 469 batches
                x = x.to(device)
                y_true = y_true.to(device)
                x = x.view(-1, 784) # flatten
                if phase == 'train':
                    optimizer.zero_grad()
                    # x.requires_grad = True # why would you do this
                ...
                nb_correct += (y_pred == y_true).sum().item() # loader is how many batches
