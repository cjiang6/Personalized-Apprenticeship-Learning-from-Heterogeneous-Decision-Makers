"""
BDT implimentation testing if the optimizer for the embedding should be together or apart
"""
...
from utils.pairwise_utils import create_new_data, create_sets_of_20_from_x_for_pairwise_comparisions, find_which_schedule_this_belongs_to, transform_into_torch_vars
...
class ProLoTrain:
    ...
    def __init__(self, num_schedules):
        ...
        self.embedding_optimizer = torch.optim.SGD([{'params': self.model.bayesian_embedding, 'lr': .0001}])
        # TODO: make optimizers separate
        ...
        while not training_done:
            ...
            for counter in range(set_of_twenty, set_of_twenty + 20):
                if counter == phi_i_num:  # if counter == phi_i_num:
                    ...
                    feature_input, P = transform_into_torch_vars(feature_input, self.distribution_epsilon, True, torch.cuda.is_available())
            ...
            for counter in range(set_of_twenty, set_of_twenty + 20):
                if counter == phi_i_num:
                    ...
                    feature_input, P = transform_into_torch_vars(feature_input, self.distribution_epsilon, False, torch.cuda.is_available())
                    output = self.model(feature_input)
                    loss = loss_func.forward(P, output, self.alpha)
    ...
    def evaluate_on_test_data(self, load_in_model=False):
        ...
        for j in range(0, num_schedules):
            ...
            prediction_accuracy = [0, 0]
        print('top1_mean for ', self.alpha, ' is : ', np.mean(percentage_accuracy_top1))
