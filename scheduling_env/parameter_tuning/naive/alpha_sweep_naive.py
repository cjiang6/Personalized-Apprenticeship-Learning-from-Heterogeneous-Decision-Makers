...
# TODO: put data generators into a separate file since they will need to be called in every naive and pairwise file
...
class BDTTrain:
    ...
    def __init__(self, alpha):
        ...
        self.X, self.Y, self.schedule_array = create_new_dataset(self.num_schedules)
    ...
    def evaluate(self, load_in_model=False):
        ...
        optimizer_for_embedding = self.opt = torch.optim.RMSprop([{'params': self.model.bayesian_embedding, 'lr': .0001}])
    ...
    def save_trained_nets(self):
        ...
        torch.save({'nn_state_dict': self.model.state_dict(),
                   ...
                   '/home/ghost/PycharmProjects/bayesian_prolo/saved_models/pairwise_saved_models/uniform_model' + str(self.alpha) + '.tar')
    ...
    def save_performance_results(self, top1, top3):
        ...
        data = {'top1_mean': np.mean(top1),
                ...
                'alpha': self.alpha}
        save_pickle(file=data, file_location=self.home_dir + '/saved_models/pairwise_saved_models/', special_string=str(self.alpha) + 'uniform_alpha.pkl')
        if np.mean(top1) > .6:
            exit()
    ...
    def find_optimal_alpha(self):
        ...
        performance_files2 = glob.glob(self.home_dir + '/saved_models/pairwise_saved_models/*.pkl')
...
def main():
    ...
    """
    trainer = None
    for alpha in np.linspace(.1, 5, num=30):
