...
class BDTTrain:
    ...
    def __init__(self, bayesian_dim):
        ...
        self.start_of_each_set_twenty = create_sets_of_20_from_x_for_pairwise_comparisions(self.schedule_array)
    ...
    def evaluate(self, load_in_model=False):
        ...
        for j in range(int(self.num_schedules * .8), self.num_schedules):
            optimizer_for_embedding = self.opt = torch.optim.SGD([{'params': self.model.bayesian_embedding, 'lr': .9}])
            load_in_embedding(self.model, self.embedding_list, j)
            schedule_bounds = self.schedule_array[j]
            step = schedule_bounds[0] # starting index of schedule within the data
            ...
            while step < schedule_bounds[1]:
                ...
                self.embedding_list = store_embedding_back(self.model, self.embedding_list, j)
                step += 20
